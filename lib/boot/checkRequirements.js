...
function getOsRequirements() {
        ...
        deps: {
            ...
            yarn: {
                ...
                alternateCmd: () => {
                    ...
                    try {
                        output = Webiny.execSync('npm ls yarn --json', {stdio: 'pipe'}).toString();
                        ...
                        if (!_.has(data, 'dependencies.yarn')) {
                            ...
                            try {
                                output = Webiny.execSync('npm ls yarn --json --global', {stdio: 'pipe'}).toString();
                                data = JSON.parse(output);
                            } catch (err) {
                                if (err.stdout) {
                                    data = JSON.parse(err.stdout);
    ...
    _.each(requirements.deps, function (config, key) {
        ...
        try {
            config.version = Webiny.execSync(config.cmd + grep, {stdio: 'pipe'}).toString().split("\n")[0];
            ...
            if (config.alternateCmd) {
                ...
                try {
                    config.version = Webiny.execSync(config.alternateCmd + grep, {stdio: 'pipe'}).toString().split("\n")[0];
                    config.satisfied = !config.reqVersion || config.version.startsWith(config.reqVersion);
                    return;
                } catch (err) {
                    // Do nothing
...
module.exports = {
    ...
    requirements: function () {
        ...
        if (!req.satisfied) {
            throw new Error('Please install the missing dependencies and run webiny-cli again!');
        }

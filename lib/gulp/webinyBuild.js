        ...
        add: function (appObj) {
            return through.obj(function (file, encoding, callback) {
                ...
                if (['js', 'css'].indexOf(type) > -1) {
                    ...
                    // If assets were not modified, the pipe will point to Apps/{app}/Js/* folder so we need to generate a new delimiter
                    var delimiter = file.path.indexOf('Apps') === -1 ? appObj.path : appObj.path.replace('/', '/Js/');
                    // Add asset to meta
            ...
            if (!config.production) {
                ...
                if (_.isArray(deletedModules) && deletedModules.length > 0) {
                    ...
                    deletedModules.forEach(function (m) {
                        utils.deleteFile(appObj.buildDir('/scripts/' + m + '.js'));
                    });
                }

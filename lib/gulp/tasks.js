...
var flushCache = function (config) {
    return 'wget --no-check-certificate -qO- ' + config.domain + '/__clear-cache__';
};
            ...
            return new Promise(function (resolve, reject) {
                ...
                try {
                    ...
                    ssh.command(structure, function () {
                        ...
                        utils.shellExecute('rsync --progress -e ssh ' + config.releasePath + ' ' + config.host + ':~/www/releases');
                        ...
                        ssh.command(unzip, function () {
                            ...
                            ssh.command(activate, function (res) {
                                ...
                                utils.info('Clearing cache for ' + utils.chalk.magenta(config.domain) + '...');
                                ssh.command(flushCache(config), function (res) {
                                    var commandOut = res.stdout || '{}';
                                    if (res.exitCode !== 0 || _.get(JSON.parse(commandOut), 'flushed') !== true) {
                                        utils.failure('Failed to flush cache!');

...
var SshClient = require('./../../node_modules/node-sshclient');
                    ...
                    scp.upload(config.releasePath, '~/www/releases/', function (procResult) {
                        ...
                        ssh.command(unzip, function () {
                            ...
                            var activate = [
                                ...
                                'mkdir -p ~/www/files/' + folder + '/{Uploads,Temp}',
                            ...
                            ssh.command(activate, function (res) {
                                ...
                                utils.info('Attempting to restart php-fpm service...');
                                var services = [
                                    'sudo /usr/sbin/service php7.0-fpm restart'
                                ].join('&&');
                                ssh.command(services, function (res) {
                                    if (res.exitCode !== 0) {
                                        utils.failure('Could not restart php-fpm service!');
                                        utils.log(res.stderr);
                                    }

                                    utils.info('Executing post-deploy scripts...');
                                    var postDeploy = [
                                        'cd ~/www/active/' + folder + ' && php vendor/webiny/setup/release.php ' + domain
                                    ].join('&&');

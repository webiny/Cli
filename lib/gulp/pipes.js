...
module.exports = function (config, $) {
    ...
    var getAssets = function (appObj, type) {
        var assets = appObj.assets.getVendors()[type].map(function (path) {
            return appObj.sourceDir + '/Assets/' + path;
        });
        ...
        return assets.map(function (file) {
    ...
    pipes.buildVendorScripts = function (appObj) {
        ...
        if (_.has(assets, 'Js')) {
            vendorsPipes.push(
                gulp.src(getAssets(appObj, 'Js'))
                    .pipe($.duration('Vendor JS'))
                    // ES6
                    .pipe(es6Filter)
                    .pipe(eslint())
                    .pipe(eslintFormat())
        ...
        if (_.has(assets, 'Styles')) {
            vendorsPipes.push(
                gulp.src(getAssets(appObj, 'Styles'))
        ...
        if (_.has(assets, 'Images')) {
            vendorsPipes.push(
                gulp.src(getAssets(appObj, 'Images'))
